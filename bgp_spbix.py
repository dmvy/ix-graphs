...
load = 'load'
enc_data = urllib.urlencode([('Submit', 1), ('query', 'summary'), ('rtr', 'spb-rs1.ripn.net')])
page = urllib.urlopen('http://www.ix.ru/spb/network/lookingglass.html', enc_data)
#print page.read()
...
#    print link.text
    for inform in link.text.split('\n'):
        ...
        if len(res)>=7:
            if len(res[0]) > 11:
                if ( res[0][0:11] == '192.226.100' ):
#                    print res[0], res[2]
                    if ( res[0][0:10] == '192.226.100' ): continue
